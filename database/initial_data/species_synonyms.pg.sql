...
-- Trigger for updated_at (先刪除再建立，確保可重複執行)
DROP TRIGGER IF EXISTS trigger_species_synonyms_updated_at ON species_synonyms;
...
CREATE TABLE IF NOT EXISTS species_merge_log (
    id SERIAL PRIMARY KEY,
    action_type VARCHAR(30),                     -- 'auto_add', 'synonym_add', 'name_normalize', 'survey_update'
    merge_type VARCHAR(20),                      -- 舊有欄位（向下相容）
    species_id VARCHAR(10),                      -- 操作的樹種 ID
...
-- 確保舊表也有新欄位（冪等升級）
ALTER TABLE species_merge_log ADD COLUMN IF NOT EXISTS action_type VARCHAR(30);
ALTER TABLE species_merge_log ADD COLUMN IF NOT EXISTS species_id VARCHAR(10);
-- 放寬 merge_type 的 NOT NULL 限制（舊表可能有 NOT NULL）
ALTER TABLE species_merge_log ALTER COLUMN merge_type DROP NOT NULL;
