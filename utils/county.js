...
// 注意：value 一律使用內政部官方 COUNTYNAME（「臺」）。matchCity 會自動處理使用者
// 輸入「台」/「臺」的等價，但所有來源的 _city 必須單一形式才能讓前端 `==` 比對成立。
...
// 縣市關鍵字 — key 一律用「臺」形式（內政部官方 COUNTYNAME）。
// value 中同時包含「台」「臺」兩種變體關鍵字以接受使用者各種輸入。
